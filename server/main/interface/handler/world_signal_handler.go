...
type WorldSignalHandler struct {
	Usecase *usecases.WorldSignalUsecase
}
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			...
			case errors.Is(err, usecases.ErrWorldSignalWindowInvalid):
				JSONError(w, http.StatusBadRequest, "VALIDATION_ERROR", err.Error(), nil)
			default:
				JSONError(w, http.StatusInternalServerError, "INTERNAL_ERROR", "failed to fetch world signals", err.Error())
			}
			return
		}
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			...
			case errors.Is(err, usecases.ErrWorldSignalLocationRequired),
				...
				JSONError(w, http.StatusBadRequest, "VALIDATION_ERROR", err.Error(), nil)
			default:
				JSONError(w, http.StatusInternalServerError, "INTERNAL_ERROR", "failed to fetch world signal from open data", err.Error())
			}
			return
		}

		JSONSuccess(w, http.StatusCreated, result, "world signals fetched and stored")
	}
}
...
func parseFromToRange(r *http.Request) (time.Time, time.Time, error) {
	...
	if from.After(to) {
		return time.Time{}, time.Time{}, usecases.ErrWorldSignalWindowInvalid
	}
	return from, to, nil
}
