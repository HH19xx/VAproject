...
func (r *analysisSnapshotRepository) FindAllByUserID(ctx context.Context, userID, limit int) ([]*domain.AnalysisSnapshot, error) {
	query := `
		SELECT
			id,
			user_id,
	...
	snapshots := make([]*domain.AnalysisSnapshot, 0)
	for rows.Next() {
		...
		if baselineCount.Valid || baselineAvg.Valid || baselineVar.Valid || baselinePrototypeRate.Valid {
			baseline := &domain.DistributionStats{}
...
func (r *analysisSnapshotRepository) Create(ctx context.Context, snapshot *domain.AnalysisSnapshot) (int, error) {
	query := `
