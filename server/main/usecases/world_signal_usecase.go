...
type WorldSignalRepository interface {
	...
	FindByRange(ctx context.Context, source, locationKey, signalType string, from, to time.Time, limit int) ([]*domain.WorldSignal, error)
}
...
func (u *WorldSignalUsecase) FetchAndStore(
	ctx context.Context,
	source string,
	locationKey string,
	...
	case "", worldSignalSourceOpenMeteo:
		return u.FetchAndStoreOpenMeteo(ctx, locationKey, latitude, longitude, pastDays, forecastDays, actor)
	case worldSignalSourceEStatDashboard:
		return u.FetchAndStoreEStatDashboard(ctx, locationKey, actor)
	default:
...
func (u *WorldSignalUsecase) ListByRange(
	...
	return u.Repo.FindByRange(ctx, source, locationKey, signalType, from, to, limit)
}

func (u *WorldSignalUsecase) FetchAndStoreEStatDashboard(
	ctx context.Context,
	locationKey string,
	actor string,
) (*WorldSignalFetchResult, error) {
...
func convertEStatDashboardPayload(
	body []byte,
	...
	signals := make([]*domain.WorldSignal, 0, len(dataObjs)*2)
	var from time.Time
	var to time.Time
