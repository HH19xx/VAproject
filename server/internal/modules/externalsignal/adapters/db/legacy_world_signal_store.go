...
type legacyWorldSignalStore struct {
	db *sql.DB
}

func newLegacyWorldSignalStore(db *sql.DB) worldSignalStore {
	return &legacyWorldSignalStore{db: db}
}

func (s *legacyWorldSignalStore) UpsertMany(ctx context.Context, signals []*externalsignaldomain.WorldSignal) (int, error) {
	if len(signals) == 0 {
		return 0, nil
	}
...
func (s *legacyWorldSignalStore) FindByRange(
	ctx context.Context,
	...
	signals := make([]*externalsignaldomain.WorldSignal, 0)
	for rows.Next() {
		var signal externalsignaldomain.WorldSignal
