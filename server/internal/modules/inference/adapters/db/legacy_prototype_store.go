...
type legacyPrototypeStore struct {
	db *sql.DB
}

func newLegacyPrototypeStore(db *sql.DB) prototypeStore {
	return &legacyPrototypeStore{db: db}
}

func (s *legacyPrototypeStore) FindAll(ctx context.Context, userID int) ([]*inferencedomain.Prototype, error) {
	...
	var prototypes []*inferencedomain.Prototype
	for rows.Next() {
		var p inferencedomain.Prototype
		if err := rows.Scan(
		...
		tagIDs, err := s.findTagIDsByPrototypeID(ctx, p.ID)
		if err != nil {
			return nil, err
		}
		p.TagIDs = tagIDs
...
func (s *legacyPrototypeStore) FindByID(ctx context.Context, id, userID int) (*inferencedomain.Prototype, error) {
	var p inferencedomain.Prototype
...
func (s *legacyPrototypeStore) Create(ctx context.Context, prototype *inferencedomain.Prototype) (int, error) {
...
func (s *legacyPrototypeStore) Update(ctx context.Context, prototype *inferencedomain.Prototype) error {
	tx, err := s.db.BeginTx(ctx, nil)
	if err != nil {
		return err
	}
...
func (s *legacyPrototypeStore) Delete(ctx context.Context, id, userID int) error {
...
func (s *legacyPrototypeStore) findTagIDsByPrototypeID(ctx context.Context, prototypeID int) ([]int, error) {
	rows, err := s.db.QueryContext(ctx, `
		SELECT tag_id
		FROM prototype_tags
...
func uniqueIntIDs(items []int) []int {
	if len(items) == 0 {
		return nil
	}
